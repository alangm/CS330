...
(check-expect (area (make-circ (make-position 0 0) 4)) (* pi 16))
...
(define (translate-shape shape delta)
  (cond
    ((circ? shape) (+ (position-x (circ-center shape)) delta))
    ((square? shape) (+ (position-x (square-upper-left shape)) delta))
    ((rect? shape) (+ (position-x (rect-upper-left shape)) delta))
    (else "Not a recognized shape")
  )
)
